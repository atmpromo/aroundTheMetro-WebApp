...
class Place_Details extends CI_Controller{
...
//		  $this->load->model('metros_model');
    ...
    function index($lang=0 , $country , $city , $placekey) {
        $data['data']['bannerimages'] = $this->bannerimages_model->getBannerimages($lang);
		$place = $this->places_model->getplacebykey($lang, $country , $city , $placekey);
...
//        $data['malls'] = $this->shoppingmalls_model->getShoppingmalls($lang);
		...
		$data['data']['city'] = $city;
		$data['city'] = $city;
        $data['main_content'] = 'place_details';
        $data['activatedMenu'] = 'placesmenu';
        $data['data']['language'] = $lang;
		$data['data']['title'] = $place['name'];
