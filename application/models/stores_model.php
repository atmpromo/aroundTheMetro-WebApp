...
class Stores_model extends CI_Model
{
    ...
    public $selects = [
                        'id, name, type, mallid, metroid, aboutus, contact, facebook, link, featured, imagename, coverphoto_filename, claimed'
                      , 'id, name_fr, type_fr, mallid, metroid, aboutus_fr, contact, facebook, link, featured, imagename, coverphoto_filename, claimed'
                      , 'id, name_cn, type_cn, mallid, metroid, aboutus_cn, contact, facebook, link, featured, imagename, coverphoto_filename, claimed'
                      , 'id, name, name_fr, name_cn, type, type_fr, type_cn, mallid, metroid, aboutus, aboutus_fr, aboutus_cn, contact, facebook, link, featured, imagename, coverphoto_filename, claimed'
                      ];
    ...
    public function getStores($lang, $type=null, $mallid=null, $metroid=null, $limit = 0)
    {
    ...
    public function getStoresByCategory($lang) {
        $this->db->select($this->selects[$lang]);
        $this->db->from($this->table_name);
        ...
        for($i=0; $i < count($stores); $i++) {
            if (strcasecmp($stores[$i]['type'], getTypenamefromType($lang, 'restaurants')) == 0) {
                array_push($restaurants, $i);
            } else if (strcasecmp($stores[$i]['type'], getTypenamefromType($lang, 'boutiques')) == 0) {
                array_push($boutiques, $i);
            } else if (strcasecmp($stores[$i]['type'], getTypenamefromType($lang, 'beautyhealths')) == 0) {
                array_push($beautyhealths, $i);
            } else if (strcasecmp($stores[$i]['type'], getTypenamefromType($lang, 'attractions')) == 0) {
